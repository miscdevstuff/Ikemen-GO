...
float getMorphTargetWeight(int idx) {
    int vecIndex  = idx / 4;              // which vec4
    int compIndex = idx - vecIndex * 4;   // 0..3
...
void main(void) {
    ...
    if (morphTargetWeight[0][0] != 0.0){
        for (int idx = 0; idx < numTargets; ++idx)
        {
            ...
            } else if (idxf < morphTargetOffset[1]){
                normal += w * vec3(COMPAT_TEXTURE(morphTargetValues, xy));
            } else if (idxf < morphTargetOffset[2]){
                tangent += w * vec3(COMPAT_TEXTURE(morphTargetValues, xy));
            } else if (idxf < morphTargetOffset[3]){
                texcoord += w * vec2(COMPAT_TEXTURE(morphTargetValues, xy));
            } else {
                vColor += w * COMPAT_TEXTURE(morphTargetValues, xy);
            }
        }
    }
    ...
    if (useJoint0){
        ...
        if (outlineAttribute.w > 0.0){
            vec3 p = normalize(mat3(normalMatrix) * outlineAttribute.xyz)
                     * outlineAttribute.w * meshOutline
                     * length(cameraPosition - tmp2.xyz);
            tmp2.xyz += p;
        } else {
